myapp.controller("menuController", function(
    ...
    function menuload() {
      ...
      var getClientModeData = function() {
        $http
          .get(
            URL +
              "/cgi_get_fillparams?Object=Device.X_INTEL_COM_ClientMode.Profile&Enable="
          )
          .success(function(data, status, headers, config) {
            if (status === 200) {
              ...
              if (data !== undefined && data.Objects !== undefined) {
                if (data.Objects.length > 0) {
                  ...
                  angular.forEach(data.Objects, function(object) {
                    if (
                      ...
                      object.Param[0] !== undefined &&
                      object.Param[0].ParamValue === "true"
                    ) {
                      isEnpointTrue = true;
                    }
                  });
                  ...
                  if (isEnpointTrue === true) {
                        ...
                        .error(function(data, status, headers, config) {
                          var getMenu = function() {
                            $http
                              .get(
                                URL +
                                  "cgi_action?Action=Menu&Language=" +
                                  language
                              )
                              .success(function(data, status, headers, config) {
                                if (status === 200) {
                    ...
                    var getBeerocksData = function() {
                      $http
                        .get(
                          URL +
                            "/cgi_get_fillparams?Object=Device.X_INTEL_COM_BEEROCKS&Gateway=&Onboarding="
                        )
                        .success(function(data, status, headers, config) {
                          if (status === 200) {
                            var responseData = data;
                            ...
                            if (
                              ...
                              if (data.Objects.length > 0) {
                                ...
                                angular.forEach(data.Objects, function(object) {
                                  angular.forEach(object.Param, function(
                                    ...
                                    if (objectparam !== undefined) {
                                      if (
                                        object.ObjName ===
                                          "Device.X_INTEL_COM_BEEROCKS" &&
                                        objectparam.ParamValue === "false" &&
                                        objectparam.ParamName === "Gateway"
                                      ) {
                                        var objectDetails = {};
                                        objectDetails.objectname =
                                          object.ObjName +
                                          "." +
                                          objectparam.ParamName;
                                        objectDetails.isEnpointTrue =
                                          objectparam.ParamValue;
                                        ...
                                        object.ObjName ===
                                          "Device.X_INTEL_COM_BEEROCKS" &&
                                        objectparam.ParamValue === "false" &&
                                        objectparam.ParamName === "Onboarding"
                                      ) {
                                        var objectDetails = {};
                                        objectDetails.objectname =
                                          object.ObjName +
                                ...
                                if (
                                  objectname.indexOf(
                                    "Device.X_INTEL_COM_BEEROCKS.Gateway"
                                  ) > -1
                                ) {
                                  ...
                                  if (
                                    objectResponse[index].isEnpointTrue ===
                                    "false"
                                  ) {
                                    var getIreMenu = function() {
                                      $http
                                        .get("menu_ire_mode.json")
                                        .success(function(
                                          data,
                                          status,
                                          headers,
                                  ...
                                  objectname.indexOf(
                                    "Device.X_INTEL_COM_BEEROCKS.Onboarding"
                                  ) > -1
                                ) {
                                  var index = objectname.indexOf(
                                    "Device.X_INTEL_COM_BEEROCKS.Onboarding"
                ...
                var getMenu = function() {
                  $http
                    .get(URL + "cgi_action?Action=Menu&Language=" + language)
                    .success(function(data, status, headers, config) {
                      if (status === 200) {
                        $scope.createmenu(data, $q, $http);
                      } else if (status === TOKEN_MISMATCH_CODE) {
              ...
              for (k = 0; k < childs.length; ++k) {
                ...
                function menuObjectCreate() {
                  if (child.view != undefined) {
                    ...
                    breadcrumbsdata[child.view] = [
                      { name: dashboardname, path: itemviewnmae },
                    ];
                    ...
                    for (var m = 0; m < voipchildrens.length; m++) {
                      if (voipchildrens[m].view != undefined) {
                        ...
                        breadcrumbsdata[voipchildrens[m].view].push({
                          ...
                          path:
                            voipchildrens[m].viewtype +
                            "/" +
                            voipchildrens[m].view,
                          index: parseInt(childs[k].order) - 1,
                          order: parseInt(voipchildrens[m].order) - 1,
                        });
                      }
                ...
                if (child.checkvalue && child.checkurl) {
                  for (p = 0; p < requestcheck.length; ++p) {
                    if (child.name == requestcheck[p]) successindex = p;
                  }
                  if (successindex > -1) {
                    if (
                      responses[successindex] != undefined &&
                      responses[successindex].data != undefined &&
                      responses[successindex].data.Objects[0].Param[0]
                        .ParamValue != child.checkvalue
                    ) {
                      menuObjectCreate();
                    }
                  }
      ...
      for (var i = 0; i < childNodes.length; i++) {
        if (childNodes[i].nodeType !== 3) {
          for (var j = 0; j < childNodes[i].childNodes.length; j++) {
            if (
              childNodes[i].childNodes[j].nodeType !== 3 &&
              childNodes[i].childNodes[j].nodeName === "A" &&
              childNodes[i].childNodes[j] !== currentNode
            ) {
  ...
  highlightMenuItem = function(currentListItem, currentNode) {
    for (var i = 0; i < currentListItem.childNodes.length; i++) {
      if (
        currentListItem.childNodes[i].nodeType !== 3 &&
        currentListItem.childNodes[i].nodeName === "A"
      ) {
        if (currentNode.className.indexOf("hide") !== -1) {
          currentListItem.childNodes[i].className = "ng-scope";
        } else {
          currentListItem.childNodes[i].className =
  ...
  $scope.formfilldata = function(object, objectparams) {
    ...
    $http
      .get(URL + "cgi_get_nosubobj?Object=" + orgobject)
  ...
  $scope.callregister = function(scopevariable, clickedtab, req) {
    ...
    var finalobject = "";
    var reqobjectsarray = [
      "Device.Services.VoiceService.VoiceProfile.1.Line.1.X_VENDOR_COM_MissCallRegister.X_VENDOR_COM_MissCallRegEntry.*",
      "Device.Services.VoiceService.VoiceProfile.1.Line.1.X_VENDOR_COM_DialCallRegister.X_VENDOR_COM_DialCallRegEntry.*",
    ...
    var reqobjectsarray1 = [
      "Device.Services.VoiceService.VoiceProfile.*.Line.*.X_VENDOR_COM_MissCallRegister.X_VENDOR_COM_MissCallRegEntry.*",
      "Device.Services.VoiceService.VoiceProfile.*.Line.*.X_VENDOR_COM_DialCallRegister.X_VENDOR_COM_DialCallRegEntry.*",
