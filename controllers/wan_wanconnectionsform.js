myapp.controller(
 "wan_wanconnectionsform",
 function ($scope, $http, $location, $routeParams, helperService) {
  // ------------------------------------------------------------
  // Unified form model
  // ------------------------------------------------------------
  $scope.form = {
  ...
  function encodeParam(value) {
   return encodeURIComponent(value === undefined || value === null ? "" : value.toString());
  }
  ...
  function loadQoSDataForLink(linkObj) {
   ...
   if (linkNumMatch) {
    ...
    if (qosObj) {
     ...
     $scope.form.peakCellRate = parseInt(getAtmParamValue(qosObj, "PeakCellRate")) || "";
     $scope.form.maximumBSize = parseInt(getAtmParamValue(qosObj, "MaximumBurstSize")) || "";
     $scope.form.sustainableCellRate =
  ...
  async function loadStaticGatewayAddress(ipInterface) {
   try {
    ...
    const response = await $http.get(URL + "cgi_get?Object=Device.Routing.Router.1.IPv4Forwarding");
    ...
    if (response.data?.Objects?.length > 0) {
     ...
     if (forwardingEntry) {
      ...
      console.log("No IPv4Forwarding entry found for interface:", normalizedInterface);
      $scope.form.gatewayaddress = "";
     }
    } else {
  ...
  async function loadIPv6Settings() {
   try {
    ...
    if (ipv6Response.data?.Objects?.length > 0) {
     ...
     $scope.form.ipv6PrimaryDNS = getAtmParamValue(ipv6Obj, "X_LANTIQ_COM_PRI_DNSv6") || "";
     $scope.form.ipv6SecondaryDNS = getAtmParamValue(ipv6Obj, "X_LANTIQ_COM_SEC_DNSv6") || "";
     $scope.form.ipv6DomainName = getAtmParamValue(ipv6Obj, "X_LANTIQ_COM_V6_DOMAIN_NAME") || "";

     // Load LAN mode (might indicate WAN mode preferences)
     const lanMode = getAtmParamValue(ipv6Obj, "X_LANTIQ_COM_IPv6_LANMode") || "";
    ...
    const interfaceResponse = await $http.get(URL + "cgi_get?Object=" + $scope.editIPInterface);
   ...
   try {
    ...
    if (interfaceResponse.data?.Objects?.length > 0) {
     ...
     if (!$scope.form.ipv6IPAcqMode) {
      ...
      const dhcpv6Response = await $http.get(URL + "cgi_get?Object=Device.DHCPv6.Client");
      ...
      if (dhcpv6Response.data?.Objects?.length > 0) {
       ...
       const dhcpClient = dhcpv6Response.data.Objects.find((client) => {
        ...
        return (
         interfaceParam &&
         interfaceParam.replace(/\.$/, "") === $scope.editIPInterface.replace(/\.$/, "")
        );
       });

       if (dhcpClient && getAtmParamValue(dhcpClient, "Enable") === "true") {
        $scope.form.ipv6IPAcqMode = "DHCPv6";
        ...
        $scope.form.ipv6RapidCommit = getAtmParamValue(dhcpClient, "RapidCommit") === "true";
       }
      }
     }
   ...
   try {
    ...
    if (dhcpv6Response.data?.Objects?.length > 0) {
     ...
     if (dhcpClient) {
      ...
      const ianaId = parseInt(getAtmParamValue(dhcpClient, "X_LANTIQ_COM_IANAID") || "0", 10);
      const iapdId = parseInt(getAtmParamValue(dhcpClient, "X_LANTIQ_COM_IAPDID") || "0", 10);
      ...
      $scope.form.ipv6RequestAddresses =
       getAtmParamValue(dhcpClient, "RequestAddresses") === "true";
      $scope.form.ipv6RequestPrefixes = getAtmParamValue(dhcpClient, "RequestPrefixes") === "true";
      $scope.form.ipv6RapidCommit = getAtmParamValue(dhcpClient, "RapidCommit") === "true";
     } else {
      console.log("No enabled DHCPv6 client found for interface:", $scope.editIPInterface);
     }
    }
   } catch (error) {
  ...
  $scope.showIPv6DNSFields = function () {
   return (
    ...
    ($scope.form.ipv6IPAcqMode === "Static" || $scope.form.ipv6IPAcqMode === "DHCPv6")
   );
  };

  $scope.showIPv4Options = function () {
   return $scope.form.protocolType === "IPv4" || $scope.form.protocolType === "IPv4/IPv6";
  };
  ...
  $scope.shouldShowIPAcqMode = function () {
   return $scope.form.encapsulationMode !== "PPPoE" && $scope.form.wanMode !== "BridgedWan";
  };

  $scope.showStaticFields = function () {
   return $scope.form.ipAcqMode === "Static" && $scope.form.wanMode !== "BridgedWan";
  };
  ...
  $scope.validateDNSForm = function () {
   ...
   const same = $scope.form.secondaryDNS && $scope.form.secondaryDNS === $scope.form.primaryDNS;
  ...
  async function loadAtmLinksAndQos() {
   ...
   try {
    ...
    $scope.atmLinksQos = objects.filter((obj) => /^Device\.ATM\.Link\.\d+\.QoS$/.test(obj.ObjName));
    ...
    if (!$scope.isEditMode && $scope.vpiVciOptions.length > 0 && !$scope.form.vpiVci) {
  ...
  async function loadBridgeConnections() {
   ...
   if ($scope.loadingBridgeConnections || $scope.bridgeConnections.length > 0) {
    return;
   }
   ...
   try {
    ...
    if (response.data?.Objects?.length > 0) {
     ...
     if (!$scope.form.selectedBridge && $scope.bridgeConnections.length > 0) {
  ...
  async function loadMACAddressFromEthernetLink(ipInterface) {
   ...
   try {
    ...
    if (ethLinkObj) {
     ...
     const macCloningEnabled = getAtmParamValue(ethLinkObj, "X_INTEL_COM_MACCloning");
  ...
  async function loadEditModeData() {
   ...
   try {
    ...
    if (response.data?.Objects?.length > 0) {
     ...
     const protocolTypeParam = getParamFromObject(ipObj, "X_LANTIQ_COM_ProtocolType");
     if (protocolTypeParam) {
      if (protocolTypeParam === "IPv4/IPv6" || protocolTypeParam === "IPv6") {
       $scope.form.protocolType = "IPv4/IPv6";
      }
     }
     ...
     if (addressingType === "X_LANTIQ_COM_PPPoE") {
      ...
      try {
       ...
       if (pppResponse.data?.Objects?.length > 0) {
        ...
        if (pppInterface) {
         ...
         let username = getParamFromObject(pppInterface, "Username") || "";
      ...
      if (ipv4Obj) {
       $scope.form.ipaddress = getAtmParamValue(ipv4Obj, "IPAddress") || "";
       $scope.form.subnetmask = getAtmParamValue(ipv4Obj, "SubnetMask") || "";
      }
     ...
     $scope.form.wanMode =
      getParamFromObject(ipObj, "X_LANTIQ_COM_WANMode") === "Bridged" ? "BridgedWan" : "RoutedWan";
     $scope.form.serviceType =
      getParamFromObject(ipObj, "X_LANTIQ_COM_ServiceType") || "TR069_Internet";

     // Load Default Gateway checkbox setting
     const defaultGatewayParam = getParamFromObject(ipObj, "X_LANTIQ_COM_DefaultGateway");
  ...
  async function loadNATSettings() {
   ...
   try {
    const response = await $http.get(URL + "cgi_get?Object=Device.NAT.InterfaceSetting");

    if (response.data?.Objects) {
     const interfaceNatSetting = response.data.Objects.find((nat) => {
      const interfaceParam = nat.Param.find((p) => p.ParamName === "Interface");
      return interfaceParam && interfaceParam.ParamValue === $scope.editIPInterface;
     });

     if (interfaceNatSetting) {
      const enableParam = interfaceNatSetting.Param.find((p) => p.ParamName === "Enable");
      ...
      if (enableParam) {
       $scope.form.enableNAT =
        enableParam.ParamValue === "true" || enableParam.ParamValue === "1" ? "1" : "0";
      }
   ...
   try {
    ...
    if (atmLinkObj) {
     ...
     if (existingLink) {
      ...
      const linkNumMatch = existingLink.ObjName.match(/Device\.ATM\.Link\.(\d+)$/);
      if (linkNumMatch) {
       const qosObjName = `Device.ATM.Link.${linkNumMatch[1]}.QoS`;
       const qosObj = $scope.atmLinksQos.find((obj) => obj.ObjName === qosObjName);
  ...
  async function traceToATMLink(objPath, visited = []) {
   try {
    ...
    if (obj.ObjName.includes("Device.ATM.Link") && !obj.ObjName.includes(".QoS")) {
     return obj;
    }
  ...
  async function loadVLANInformation(ipObj) {
   try {
    ...
    if (vlanLayer) {
     ...
     // Get VLAN ID
     const vlanRes = await $http.get(`${URL}cgi_get_nosubobj?Object=${vlanLayer}`);
  ...
  function buildRequest() {
   ...
   if (isATM) {
    ...
    if (isNewVpiVci || !$scope.form.selectedATMLink) {
     ...
     request += `Object=Device.ATM.Link&Operation=Add&Enable=true&Alias=${encodeParam(atmAlias)}`;
     ...
     if ($scope.form.sustainableCellRate)
      request += `&SustainableCellRate=${encodeParam($scope.form.sustainableCellRate)}`;
     request += `&`;
    } else {
     ...
     if (hasQoSParams) {
      ...
      const linkNumMatch = $scope.form.selectedATMLink.ObjName.match(/Device\.ATM\.Link\.(\d+)$/);
      if (linkNumMatch) {
   ...
   if ($scope.form.wanMode === "BridgedWan") {
    ...
    if ($scope.form.enableVlan == "1") {
     ...
     if ($scope.form.ipAcqMode === "PPPoE" && $scope.form.protocolType !== "IPv6") {
   ...
   if ($scope.form.macCloneEnabled && $scope.form.mac_address) {
    request += `&X_INTEL_COM_MACCloning=true&MACAddress=${encodeParam($scope.form.mac_address)}`;
   } else {
   ...
   // ==================== PPPoE ====================
   if ($scope.form.ipAcqMode === "PPPoE" && $scope.form.wanMode !== "BridgedWan") {
    ...
    request += `&Username=${encodeParam($scope.form.username + "@tedata.net.eg")}`;
   ...
   if ($scope.form.wanMode === "BridgedWan") {
    const bridgeId = $scope.form.selectedBridge?.id || 1;
    const bridgePortAlias = `cpe-WEB-BridgingBridge${bridgeId}Port-${randomValue}`;
    request += `Object=${encodeParam($scope.form.selectedBridge.objName)}.Port&Operation=Add`;
   ...
   if ($scope.form.ipAcqMode === "DHCP" && $scope.form.wanMode !== "BridgedWan") {
    // DHCPv4 Client
   ...
   if ($scope.form.protocolType === "IPv4/IPv6" || $scope.form.protocolType === "IPv6") {
    ...
    if ($scope.form.ipv6IPAcqMode === "DHCPv6") {
     ...
     request += `&RequestAddresses=${$scope.form.ipv6RequestAddresses ? "true" : "false"}`;
     request += `&RequestPrefixes=${$scope.form.ipv6RequestPrefixes ? "true" : "false"}`;
     request += `&RapidCommit=${$scope.form.ipv6RapidCommit ? "true" : "false"}`;
     ...
     request += `&`;
    } else if ($scope.form.ipv6IPAcqMode === "Static" && $scope.form.ipv6StaticAddress) {
     ...
     request += `&Alias=${encodeParam(`cpe-WEB-IPv6Address-${randomValue}`)}`;
     request += `&`;
     ...
     if ($scope.form.ipv6StaticGateway) {
      ...
      request += `&Alias=${encodeParam(`cpe-WEB-IPv6Forwarding-${randomValue}`)}`;
      request += `&`;
     }
    }
    ...
    if ($scope.form.ipv6PrefixAcqMode === "Static" && $scope.form.ipv6StaticPrefix) {
     const prefixWithMask =
      $scope.form.ipv6StaticPrefix + "/" + ($scope.form.ipv6StaticPrefixMask || "64");
    ...
    if ($scope.form.ipv6PrimaryDNS || $scope.form.ipv6SecondaryDNS || $scope.form.ipv6DomainName) {
     request += `Object=Device.IPv6&Operation=Modify`;

     if ($scope.form.ipv6PrimaryDNS) {
      request += `&X_LANTIQ_COM_PRI_DNSv6=${encodeParam($scope.form.ipv6PrimaryDNS)}`;
     }

     if ($scope.form.ipv6SecondaryDNS) {
      request += `&X_LANTIQ_COM_SEC_DNSv6=${encodeParam($scope.form.ipv6SecondaryDNS)}`;
     }

     if ($scope.form.ipv6DomainName) {
      request += `&X_LANTIQ_COM_V6_DOMAIN_NAME=${encodeParam($scope.form.ipv6DomainName)}`;
     }
     request += `&`;
    }
    ...
    if ($scope.form.wanMode !== "BridgedWan") {
     request += `Object=Device.Routing.Router.1.IPv6Forwarding&Operation=Add`;
     request += `&Interface=Device.IP.Interface.${ipAlias}`;
     request += `&Alias=${encodeParam(`cpe-WEB-IPv6Forwarding-default-${randomValue}`)}`;
     request += `&`;
    }
   }

   // ==================== Static IPv4 ====================
   if ($scope.form.ipAcqMode === "Static" && $scope.form.wanMode !== "BridgedWan") {
    // IPv4 Address
    ...
    if ($scope.staticDNSData.length > 0) {
     $scope.staticDNSData.forEach((dns, idx) => {
      ...
      request += `&Interface=Device.IP.Interface.${ipAlias}`;
      request += `&Alias=${encodeParam(`cpe-WEB-DNSServer-${randomValue}-${idx}`)}`;
      request += `&`;
     });
    }
   }
  ...
  function detectSelectiveChanges() {
   ...
   if ($scope.form.encapsulationMode === "PPPoE" && $scope.form.ipAcqMode === "PPPoE") {
    if (
   ...
   if ($scope.form.defaultGateway !== $scope.originalValues.defaultGateway) {
  ...
  async function buildSelectiveModifyRequest(changes) {
   ...
   try {
    ...
    const ipResponse = await $http.get(URL + "cgi_get?Object=" + $scope.editIPInterface);
    ...
    console.error("Error building selective modify request:", error);
    return "";
   }
  ...
  async function deleteOldConnection() {
   ...
   try {
    ...
    const res = await $http.get(`${URL}cgi_get_nosubobj?Object=${$scope.editIPInterface}`);
    ...
    if (mainObj) {
     ...
     async function collectLowerLayers(obj) {
      ...
      if (lowerParam?.ParamValue) {
       ...
       try {
        const lowerRes = await $http.get(`${URL}cgi_get_nosubobj?Object=${cleanLower}`);
     ...
     for (let i = uniqueLayers.length - 1; i >= 0; i--) {
      deleteRequest += `Object=${encodeParam(uniqueLayers[i])}&Operation=Del&`;
     }
    }
   } catch (error) {
  ...
  async function findAssociatedObjects(mainObj) {
   ...
   try {
    ...
    const pppRes = await $http.get(URL + "cgi_get?Object=Device.PPP.Interface");
    ...
    // Find Ethernet Link
    const ethRes = await $http.get(URL + "cgi_get?Object=Device.Ethernet.Link");
    ...
    // Find NAT settings
    const natRes = await $http.get(URL + "cgi_get?Object=Device.NAT.InterfaceSetting");
    ...
    // Find DHCP client
    const dhcpRes = await $http.get(URL + "cgi_get?Object=Device.DHCPv4.Client");
  ...
  $scope.submit = async function () {
   ...
   if ($scope.form.accessType === "ATM") {
    ...
    if ($scope.customWanForm.atmForm && !$scope.customWanForm.atmForm.$valid) {
   ...
   if ($scope.form.ipAcqMode === "Static") {
    if (!$scope.form.ipaddress || !$scope.form.subnetmask || !$scope.form.gatewayaddress) {
     alert("Please fill all static IP fields (IP Address, Subnet Mask, Gateway Address).");
     return;
    }
   }
   ...
   if ($scope.form.showIPv6Options) {
    ...
    if ($scope.form.ipv6PrefixAcqMode === "Static") {
     ...
     if (
      !$scope.patterns.ipv6Prefix.test(
       $scope.form.ipv6StaticPrefix + "/" + $scope.form.ipv6StaticPrefixMask
      )
     ) {
    ...
    // Validate IPv6 DNS
    if ($scope.form.ipv6PrimaryDNS && !$scope.patterns.ipv6.test($scope.form.ipv6PrimaryDNS)) {
     alert("Please enter a valid IPv6 Primary DNS address.");
     return;
    }
    if ($scope.form.ipv6SecondaryDNS && !$scope.patterns.ipv6.test($scope.form.ipv6SecondaryDNS)) {
   ...
   try {
    ...
    if ($scope.isEditMode) {
     ...
     editStrategy = detectedChanges.hasSelectiveChanges ? "selective-modify" : "full-replace";
    }
    ...
    if (editStrategy === "selective-modify") {
     ...
     if (modifyRequest) {
      ...
      if (modifyResult.status === 200) {
       ...
       if (detectedChanges.callbacks && detectedChanges.callbacks.length > 0) {
        ...
        for (const callback of detectedChanges.callbacks) {
         try {
          ...
          console.error("Callback " + callback.type + " failed:", callbackError);
         }
        }
       }
       ...
       const errorMsg =
        modifyResult.data?.Objects?.[0]?.Param?.[0]?.ParamValue || "Failed to apply modifications.";
       alert(errorMsg);
       return;
      }
     }
    ...
    if (result.status === 200) {
     ...
     if ($scope.form.encapsulationMode === "PPPoE") {
      if ($scope.form.isUserDefinedDNS && $scope.form.primaryDNS) {
       const dnsRequest = `UsrDefDNS1=${encodeParam($scope.form.primaryDNS)}`;

       if ($scope.form.secondaryDNS && $scope.form.secondaryDNS.trim() !== "") {
        dnsRequest += `&UsrDefDNS2=${encodeParam($scope.form.secondaryDNS)}`;
       }
       ...
       try {
        await $http.post(URL + "cgi_setUserDefinedDNS", "UsrDefDNS1=&UsrDefDNS2=", {
         headers: {
          "Content-Type": "application/x-www-form-urlencoded",
         },
        });
     ...
     if ($scope.form.protocolType === "IPv6") {
      ...
      try {
       ...
       const ipResponse = await $http.get(URL + "cgi_get?Object=Device.IP.Interface");
       ...
       if (ipResponse.data?.Objects?.length > 0) {
        ...
        if (exactAliasMatch) {
         ...
         const aliasPatternMatch = ipResponse.data.Objects.find((obj) => {
          const aliasParam = obj.Param?.find((p) => p.ParamName === "Alias");
          return aliasParam?.ParamValue?.includes("cpe-WEB-IPInterface-");
         });
         ...
         if (aliasPatternMatch) {
          ...
          ipResponse.data.Objects.forEach((obj) => {
           const match = obj.ObjName.match(/Device\.IP\.Interface\.(\d+)/);
           if (match) {
        ...
        if (targetInterface) {
         ...
         const disableResult = await $http.post(URL + "cgi_set", disableIPv4Request);

         if (disableResult.status === 200) {
          console.log("Successfully disabled IPv4 on interface:", targetInterface);
         } else {
          console.warn("Failed to disable IPv4 on interface:", disableResult.data);
         }
        } else {
         console.warn("Could not find the created IP interface to disable IPv4");
        }
       } else {
     ...
     const errorMsg = result.data?.Objects?.[0]?.Param?.[0]?.ParamValue || "Something went wrong.";
     alert(errorMsg);
    }
   } catch (error) {
