...
myapp.controller("firewallController", function(
  $scope,
  $timeout,
  firewallService,
  $http,
  ...
  $scope.firewallLevel = "";
  $scope.firewallLevels = ["High","Low"];
  $rootScope.$watch("firewall", function() {
    $scope.DeviceFirewall.Enable = $rootScope.firewall;
    console.info(
      "value returned to controller rootscope changed.",
      $scope.DeviceFirewall.Enable
    );
  ...
  var jsonpromise = $interval(function() {
    if (jsonloadstatus) {
      if (breadcrumbsdata[$route.current.params.param] == undefined) {
        ...
        if (localStorage.getItem("hybrideditObject") == null)
          $rootScope["breadcrumbs"].push({ name: "Add", path: "nothing" });
        else $rootScope["breadcrumbs"].push({ name: "Edit", path: "nothing" });
      } else {
        $rootScope["breadcrumbs"] =
          breadcrumbsdata[$route.current.params.param];
      ...
      .success(function(data, status, headers, config) {
        if (status === 200) {
          ...
          for (var pa1 = 0; pa1 < objectParamValues.length; pa1++) {
            ...
            if (
              $scope[
                objects[0].ObjName.replace(/\./g, "").replace(/\*/g, "")
              ] === undefined
            )
              $scope[
                objects[0].ObjName.replace(/\./g, "").replace(/\*/g, "")
          ...
          $scope.firewallLevel = objects[0].Param.find((x) => x.ParamName === "Config").ParamValue;
  ...
  $scope.changedValue = function(objectname, paramname) {
    ...
    if (modelValue === "High") {
      $timeout(function() {
        for (i = 3; i <= 13; i++) {
          getTableData(
            "cgi_get?Object=Device.Firewall.Chain.1.Rule" + "." + i,
            "DeviceFirewallChain1Rule"
          );
        }
      }).then(function() {
  ...
  function loadTableDetails() {
    ...
    for (var obj = 0; obj < $scope.mainArray.length; obj++) {
      ...
      if (tempObjectcreationstatus) $scope.tableArray.push(tempObject);
    }
  }
        ...
        if (status === 200) {
          ...
          function sortByDigits(array) {
            ...
            array.sort(function(a, b) {
              return (
                parseInt(a.ObjName.replace(re, ""), 10) -
                parseInt(b.ObjName.replace(re, ""), 10)
              );
            });
            return array;
          }
  ...
  function hasObjectWithTarget(cgiString, objectName, targetValue) {
    ...
    const pattern = new RegExp(
      `Object=${escapedObject}.*?&Target=${escapedTarget}`
    );
  ...
  $scope.firewallApply = async function() {
    ...
    $("#ajaxLoaderSection").show();
    var urlstatus = false;
    const mountedDevicesRes = await $http.get(
      URL + "cgi_get?Object=Device.USB.X_LANTIQ_COM_MountedDevice.MountedDevice"
    );

    const hasObjectTarget = (cgiString, obj, target) => {
      return (
        cgiString.includes(`Object=${obj}`) &&
        cgiString.includes(`Target=${target}`) &&
        cgiString.indexOf(`Object=${obj}`) <
          cgiString.indexOf(`Target=${target}`)
      );
    };
    ...
    post += "&Config=" + $scope.firewallLevel;
    ...
    if (hasObjectTarget(post, "Device.Firewall.Chain.1.Rule.3", "Accept")) {
      if (
        !mountedDevicesRes.data ||
        mountedDevicesRes.data.Objects?.length <= 0
      ) {
        alert(
          "Can not apply FTP rule, as there are no mounted USB devices detected"
        );
